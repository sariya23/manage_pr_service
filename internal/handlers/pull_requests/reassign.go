...
func (i PullRequestImplementation) PostPullRequestReassign(w http.ResponseWriter, r *http.Request) {
	...
	var rq api.PostPullRequestReassignJSONRequestBody
	...
	if msg, valid := pull_request_validators.ValidatePullRequestReassignRequest(rq); !valid {
	...
	pr, newReviewer, err := i.prService.Reassign(ctx, rq.PullRequestId, rq.OldUserId)
